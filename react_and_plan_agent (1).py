    ...
    def agent_loop() -> Solver:
        async def solve(state: TaskState, generate: Generate) -> TaskState:
            ...
            while not state.completed:
                if turn >= max_turns:
                    transcript().info(f"Max turns ({max_turns}) reached. Exiting...")
                    break
...
async def forward(
    ...
    while retries > 0:
        ...
        if not isinstance(message, ChatMessageAssistant):
            raise ValueError(
                f"Expected output message to be a ChatMessageAssistant. Output type: {type(message)}. Output message: {message}"
            )

        if not isinstance(message.tool_calls, list):
            raise ValueError(
                f"Expected output message tool calls to be a list. Tool calls type: {type(message.tool_calls)}. Output message: {message}"
            )
        ...
        if len(message.tool_calls) > 1:
            transcript().info(
                f"Expected output message to have exactly one tool call. Number of tool calls: {len(message.tool_calls)}. Output message: {message}. Selecting the first tool call."
            )
